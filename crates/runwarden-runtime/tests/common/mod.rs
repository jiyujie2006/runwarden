...
use runwarden_kernel::session::{
    AuthoritySnapshot, BudgetSnapshot, EmailAuthority, EvidenceAuthority,
};
...
use runwarden_state::{
    ApprovalRecordV1, CreateOperationOutcome, JournalError, NewApproval, NewOperation,
    OperationRuntimeSnapshot, RecordPolicyInput, SessionRecord, StateStore,
};
use serde_json::json;
...
impl RuntimeFixture {
    ...
    pub fn new_with_mode(enforcement_mode: EnforcementMode) -> Self {
        ...
        let story = persist_story_with_session_mode(
            &store,
            "primary",
...
pub fn persist_story_with_session_mode(
    ...
    let story = SecurityStory {
        ...
        authority: AuthoritySnapshot {
            ...
            expires_at,
            allowed_providers: vec![EMAIL_PROVIDER.to_owned()],
            files: Vec::new(),
            networks: Vec::new(),
...
impl ProviderExecutor for RecordingExecutor {
    ...
    fn reconcile(
        &self,
        _operation_id: runwarden_kernel::story::OperationId,
    ) -> ReconciliationResult {
