...
fn session_from_assessment_manifest_builds_enforcing_policy() {
    ...
    let session = SessionManifest::from_assessment("enterprise_ops", &manifest);

    assert_eq!(session.session_id, "enterprise_ops");
    assert_eq!(
        session.allowed_providers,
        vec!["runwarden.evidence.inspect"]
    );
    ...
    registry.register(provider("runwarden.evidence.inspect"));
    registry.register(provider("external.shell.command"));
    let mut enforcer = KernelEnforcer::new(registry, session.to_kernel_policy());

    let allowed = enforcer.evaluate_call(&call("runwarden.evidence.inspect", "authz-active"));
    assert_eq!(allowed.decision, PolicyDecision::Allowed);

    let denied = enforcer.evaluate_call(&call("external.shell.command", "authz-active"));
...
fn session_policy_denies_revoked_authz_from_manifest() {
    ...
    let outcome = enforcer.evaluate_call(&call("runwarden.evidence.inspect", "authz-revoked"));
