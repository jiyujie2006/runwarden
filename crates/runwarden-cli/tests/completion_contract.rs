...
#[test]
fn eval_scenarios_json_runs_checked_in_golden_corpus_suite() {
    let output = Command::new(env!("CARGO_BIN_EXE_runwarden"))
        .current_dir(workspace_root())
    ...
    let stdout = String::from_utf8(output.stdout).expect("utf8 stdout");
    assert!(stdout.contains(r#""passed": true"#));
    assert!(stdout.contains(r#""suite": "scenario-golden-corpus""#));
    assert!(stdout.contains("enterprise-agent-security"));
    assert!(stdout.contains("local-web-risk"));
    assert!(stdout.contains(r#""side_effect_executed": false"#));
}
