...
#[test]
fn check_config_and_cert_agent_config_share_findings() {
    let dir = tempdir().expect("tempdir");
    let path = dir.path().join("unsafe-runwarden.json");
    fs::write(
        &path,
    ...
    let check_output = Command::new(env!("CARGO_BIN_EXE_runwarden"))
        .args(["agent", "check-config", "--client", "claude", "--input"])
        .arg(&path)
        .arg("--json")
        .output()
        .expect("run check-config");
    let cert_output = Command::new(env!("CARGO_BIN_EXE_runwarden"))
    ...
    assert!(!cert_output.status.success());
    let check_json: serde_json::Value =
        serde_json::from_slice(&check_output.stdout).expect("check json");
    let cert_json: serde_json::Value =
        serde_json::from_slice(&cert_output.stdout).expect("cert json");

    assert_eq!(check_json["safe"], cert_json["passed"]);
    assert_eq!(check_json["findings"], cert_json["findings"]);
}
