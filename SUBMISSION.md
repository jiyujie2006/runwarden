...
- 智能体攻击脚本: proxy-probe + output-probe + agent-drive, 见 `redteam/run.py`。
...
| Scenario | 攻击面 | 期望决策 |
| --- | --- | --- |
| `prompt-injection-file-exfil` | 提示注入诱导文件外传 | inspect -> read review -> API denied |
...
1. Prompt injection scenario 展示工具路径被 Runwarden 收敛后逐步审计；proxy-probe 额外覆盖模型输入侧 `input_blocked`。
