...
Manual LLM proxy runs may write `artifacts/llm-proxy/trace.jsonl` as
...
`artifacts/redteam/proxy-trace.jsonl`. Tool-call expectations in mixed corpora
are marked `SKIP` by proxy-probe; `agent-drive` writes
...
`bash scripts/contest_bundle.sh` runs the local gate, runs the deterministic
`proxy-probe` corpus set, and writes `artifacts/contest-bundle/` with a static
`manifest.json` and `SHA256SUMS`. The bundle copies only whitelisted
submission paths and excludes `.env`, `target/`, and `node_modules/`.
