...
- pending approval records are rendered with provider, action, actor, authz,
  argument hash, risk chip, pending count, and reviewer controls. When the
  local API is running and the reviewer supplies the launch token, approval
  forms submit approve or deny decisions to `/approvals/{id}/{decision}` with
  JSON reviewer/reason payloads.
