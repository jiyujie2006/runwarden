...
- **工具调用面**：MCP 内核 `KernelEnforcer::evaluate_call` 策略门（allow/deny/ask + 单次绑定审批 + egress 拒绝 + scoped-root 包含），后接本地沙箱执行（file/email-mbox/memory/knowledge）；API/browser provider 在本地演示中是安全模拟 replay。
- **异常检测**：行为基线（provider bigram + per-provider 参数上限 + 良性 egress 主机集），偏离打分（异常序列 / 新颖 egress / 异常参数）。

## 2. 攻击场景分析（5 类，含对抗样本集 + 攻击脚本）

对抗样本集 `redteam/corpora/*.jsonl`（13 个文件、92 条手写样本，每条带 `expected`）；攻击脚本 `redteam/run.py`（`proxy-probe` + `output-probe` + `agent-drive` 三模式）。

### 2.1 场景一：提示注入 / 越狱（模型调用面）

**对抗样本集**：`prompt_injection.jsonl`（10 条）+ `jailbreak.jsonl`（9 条）。样例：
...
**证据**：proxy-probe 覆盖模型输入过滤 corpus；output-probe 覆盖输出过滤 corpus；工具、路径、记忆、知识和 egress 由五个 scenario replay 覆盖。deterministic 结果写入 `artifacts/redteam/*-summary.json` 和最终包 `redteam-results/SUMMARY.md`。`agent-drive` 是可选 OpenCode 实机驱动，只有生成 `artifacts/redteam/agent-drive-summary.json` 时才作为本机补充证据引用。

**局限**：当前 L2 是词形相似原型，不是 embedding 模型；同义改写的泛化能力仍有限。

### 2.2 场景二：工具调用劫持（工具调用面）

**对抗样本集**：`tool_hijack.jsonl`（6 条）。样例：
...
**证据**：正式场景 `path-escape-file-boundary` 记录 `external.mcp.filesystem.read_file` → `decision:denied`、`error_kind:root_escape`、`reason:requested path escapes the configured root`、`side_effect_executed:false`、trace `obs_path_escape_denied`。可选 `agent-drive` 可用 OpenCode 复测该路径，但不作为 bundle 固定通过条件。
...
# 将 examples/agent-configs/opencode.runwarden-only.json 放入干净 OpenCode 项目或 XDG config；
# runwarden-proxy provider baseURL → http://127.0.0.1:8787/v1；
